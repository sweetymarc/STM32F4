...
#define SET_BIT_NRF(pos) ((uint8_t) (1U<<( (uint8_t) (pos) )))
...
{
  hal_nrf_write_reg (STATUS, SET_BIT_NRF(int_source));
}
...
void hal_nrf_open_pipe(hal_nrf_address_t pipe_num, bool auto_ack)
{
  ...
  switch(pipe_num)
  {
    ...
    case HAL_NRF_PIPE4:
    case HAL_NRF_PIPE5:
      en_rxaddr.value = en_rxaddr.value | SET_BIT_NRF(pipe_num);

      if(auto_ack)
      {
        en_aa.value = en_aa.value | SET_BIT_NRF(pipe_num);
      }
      else
      {
        en_aa.value = en_aa.value & (uint8_t)~SET_BIT_NRF(pipe_num);
      }
      break;
...
void hal_nrf_close_pipe(hal_nrf_address_t pipe_num)
{
  ...
  switch(pipe_num)
  {
    ...
    case HAL_NRF_PIPE4:
    case HAL_NRF_PIPE5:
      en_rxaddr.value = en_rxaddr.value & (uint8_t)~SET_BIT_NRF(pipe_num);
      en_aa.value = en_aa.value & (uint8_t)~SET_BIT_NRF(pipe_num);
      break;
...
#define NRF_WRITE_MULTIBYTE_REG_COMMON_BODY \
  do \
  { \
		HAL_SPI_TransmitReceive(&SpiHandle, (uint8_t*)buf++, (uint8_t *)&dummy, 1, 5000); \
  } while (--length);
/*lint -esym(550,dummy) symbol not accessed*/ \
/*lint -esym(438,dummy) last assigned value not used*/ \
/*lint -esym(838,dummy) previously assigned value not used*/ \
